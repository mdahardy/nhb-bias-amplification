...
const resampling_condition = sessionStorage.getItem('resampling_condition');
const socialData = JSON.parse(sessionStorage.getItem('socialDataForParticipant'));
...
function get_k_chose_green(trial_info) {
  if (resampling_condition == 'none') {
    return socialData[trial_info.stimulus_id][resampling_condition]
  }
  const stimulus_str = trial_info.stimulus_id.replace('stimulus', 's');
  const prob_green = socialData[stimulus_str][resampling_condition[0]];
  let num_green = 0;
  for (let i = 0; i < 8; i++) {
    if (Math.random() < prob_green) num_green++;
  }
  return num_green;
}
...
function create_trial() {
  ...
  if (!is_asocial) k_chose_green = get_k_chose_green(curr_trial)
...
function report(choice) {
  ...
  const social_string = is_asocial ? 'asocial' : 'social';
  ...
  const contents = {
    ...
    is_practice,
    payout_condition,
    front_end_condition: `${social_string}-${resampling_condition}`,
    is_asocial,
    randomization_color,
    proportion_green,
    k_chose_green,
