### --- Script for running the perceptual priming analyses for experiment 1 --- ###
library(lme4)
library(dplyr)
...
tie_data =  load_e1_data() %>%
  subset(social_info_green == 0.5) %>%
  subset(condition == 'social_bias')

base_dir = 'stats/e1_perceptual_priming'
